...
import asyncio
from codebase_rag.services.trace_manager import TraceManager, PrioritizedTask
from codebase_rag.services.trace_protocol import (
    Task,
    TraceResult,
    TraceStatus,
    GraphEdge,
...
class TestGraphMerging:
    ...
    def test_multiple_edges(self):
        ...
        assert nx.has_path(manager.graph, "main", "gpu_malloc")
...
class TestResultProcessing:
    ...
    def test_process_done_result(self):
        ...
        # Run async process_result synchronously
        asyncio.run(manager.process_result(result))
        
        assert len(manager.completed_results) == 1
        assert manager.stats["tasks_completed"] == 1
    
    def test_process_need_help_result(self):
    ...
    def test_cycle_prevents_spawn(self):
    ...
    def test_max_depth_prevents_spawn(self):
...
class TestBudget:
    ...
    def test_budget_exhausted(self):
        ...
        asyncio.run(manager.process_result(result1))
